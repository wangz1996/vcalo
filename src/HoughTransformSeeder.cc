...
#include "HoughTransformSeeder.hh"
...
#include "ProtoTrack.hh"
#include "SimSeed.hh"
#include "WhiteBoard.hh"
#include "DefaultHoughFunctions.hh"
...
static inline std::string to_string(std::vector<T> v);
 thread_local std::vector<std::shared_ptr<HoughTransformSeeder::HoughMeasurementStruct>>
    houghMeasurementStructs;


HoughTransformSeeder::HoughTransformSeeder(
    HoughTransformSeeder::Config cfg, Acts::Logging::Level lvl, std::vector<std::shared_ptr<HoughTransformSeeder::HoughMeasurementStruct>> houghStructs)
    : IAlgorithm("HoughTransformSeeder", lvl),
      m_cfg(std::move(cfg)),
      m_logger(Acts::getDefaultLogger("HoughTransformSeeder", lvl)) {
  houghMeasurementStructs = houghStructs;
...
ProcessCode HoughTransformSeeder::execute(
...
HoughHist
HoughTransformSeeder::createLayerHoughHist(unsigned layer,
                                                         int subregion) const {
  HoughHist houghHist(m_cfg.houghHistSize_y,
  ...
  for (unsigned index = 0; index < houghMeasurementStructs.size(); index++) {
    ...
    for (unsigned y_ = 0; y_ < m_cfg.houghHistSize_y; y_++) {
      ...
      // if(y_==0)std::cout<<y_bin_min<<" "<<xBins.first<<" "<<xBins.second<<std::endl;
...
HoughHist HoughTransformSeeder::createHoughHist(
    int subregion) const {
  HoughHist houghHist(m_cfg.houghHistSize_y,
                                    m_cfg.houghHistSize_x);
  std::cout<<"Create Hough Hist"<<std::endl;
  for (unsigned i = 0; i < m_cfg.nLayers; i++) {
    HoughHist layerHoughHist = createLayerHoughHist(i+1, subregion);
    for (unsigned x = 0; x < m_cfg.houghHistSize_x; ++x) {
      for (unsigned y = 0; y < m_cfg.houghHistSize_y; ++y) {
...
bool HoughTransformSeeder::passThreshold(
...
double HoughTransformSeeder::yToX(double y, double r,
                                                double phi) const {
  double d0 = 0;  // d0 correction TO DO allow for this
  // std::cout<<"yToX "<<" "<<std::endl;
  double x =
      asin(r * HoughTransformSeeder::m_cfg.kA * y - d0 / r) + phi;
  // std::cout<<"yToX "<<" x"<<std::endl;
  // if (1||m_cfg.fieldCorrector.connected()) {
...
// below.
unsigned HoughTransformSeeder::getExtension(
...
void HoughTransformSeeder::addSpacePoints(
    const AlgorithmContext& ctx) const {
...
void HoughTransformSeeder::addMeasurements(
    const AlgorithmContext& ctx) const {
